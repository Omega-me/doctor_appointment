...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const data = await prisma.doctor_info.findUnique({
    where: {
      id: Number(params.id),
    },
    include: {
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (restrictTo([user_role.Admin, user_role.Doctor], decodedToken.decoded?.role as user_role)) {
    return sendError('You are not allowed', eStatusCode.FORBIDDEN);
  }

  const body = await req.json();

  const customer = await prisma.doctor_info.findFirst({
    where: {
      id: Number(params.id),
    },
  });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const user = await prisma.doctor_info.delete({
    where: {
      id: Number(params.id),
    },
  });
